...
# header
st.title("Information extraction with SpaCy and BERTopic")
st.markdown("""
...
def getTextFile(data):
  text = ""
  num_topics = len(data.get_topic_info())
  for n in range(0,num_topics):
    if data.get_topic(n) != False:
      text += f"Topic {n+1},"
      for t in data.get_topic(n):
        text += f"{t[0]},{round(t[1],3)},"
  return text
...
try:
    with st.expander("Click to show more"):
        if pick_model == "BERTopic":
            ...
            text_file = getTextFile(topic_model)
            col2.download_button('Download at .txt file', text_file)
except:
    with st.expander("Click to show more"):
